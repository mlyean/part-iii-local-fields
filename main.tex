...
Let $K$ be a local field and $L/K$ a (not necessarily finite) separable extension.

\begin{definition}\phantom{}
    \begin{enumerate}
        \item $L/K$ is \emph{unramified} if $F/K$ is unramified for all finite subextension $F/K$.
        \item $L/K$ is \emph{totally ramified} if $F/K$ is totally ramified for all finite subextension $F/K$.
    \end{enumerate}
\end{definition}
...
\end{proposition}
\begin{proof}
    Every finite subextension $F/K$ is unramified, hence Galois. Thus $L/K$ is normal and and separable, hence also Galois. Moreover, there is a commutative diagram
    \begin{equation*}
...
Let $L/K$ be Galois. Then there is a surjection
\begin{equation*}
    \res : \Gal(L/K) \twoheadrightarrow \Gal(K_0/K) \xrightarrow{\sim} \Gal(k_L/k)
\end{equation*}
(c.f. \autoref{cor:13_5}). We define the inertia subgroup to be $I_{L/K} = \ker(\res)$.
...
\begin{warning}
    If $k_L/k$ is infinite, this is not the same as the subspace topology on $W(L/K) \subseteq \Gal(L/K)$, $W(L/K)$ is finer than the subspace topology. For example, $I_{L/K}$ is not open in the subspace topology.
\end{warning}
...
\begin{proposition}\label{prop:16_8}
    ...
    \begin{enumerate}
        ...
        \item If $F/K$ is a finite Galois subextension of $L/K$, then $W(L/K)/W(L/F) \cong \Gal(F/K)$.
    \end{enumerate}
\end{proposition}
\begin{proof}\phantom{}
    \begin{enumerate}
        \item We have $W(L/K)$ is dense in $\Gal(L/K)$ $\iff$ for all finite Galois subextensions $F/K$, $W(L/K)$ intersects every coset of $\Gal(L/F)$ $\iff$ for all finite Galois subextensions $F/K$, $W(L/K) \twoheadrightarrow \Gal(F/K)$ is surjective.
            ...
            \begin{equation*}
            \begin{tikzcd}[column sep=tiny]
                \Gal(L/K_0) \arrow[rr] \arrow[rd, twoheadrightarrow] & & \Gal(F/K_0 \cap F) \\
                & \Gal(K_0 F / F) \arrow[ur, "\sim"]&
            \end{tikzcd}
            \end{equation*}
            implies that $a$ is surjective. Since $\Gal(k_F/k)$ is generated by $\Frob_{k_F/k}$, $c$ is surjective.

            A diagram chase shows that $b$ is also surjective.
    \end{enumerate}
    \renewcommand{\qedsymbol}{}
\end{proof}
