...
\begin{definition}\phantom{}
    \begin{itemize}
        ...
        \item The \emph{integral closure} $R^{\Int(S)}$ of $R$ in $S$ is defined to be $R^{\Int(S)} = \{s \in S \mid s \text{ is integral over } R\}$.
...
\begin{proposition}
    Let $(K, \abs{\cdot})$ be a non-archimedean completely valued field. The following are equivalent:
    \begin{enumerate}
...
\begin{proof}
    ...
    (ii) $\Rightarrow$ (i): For $a \in K$, $a + \cO_K$ is a compact neighbourhood of $a$.

    (ii) $\Rightarrow$ (iii): Let $x \in \fm$ and $A_x$ be a set of coset representatives for $\cO_K / x \cO_K$. Then $\cO_K = \bigcup_{y \in A_x} y + x \cO_K$ is a disjoint open cover. Thus $A_x$ is finite by compactness, and so $\cO_K / x \cO_K$ is finite. It follows that $k$ is finite, since we have a surjection $\cO_K / x \cO_K \to k$.
