...
\end{theorem}
\begin{proof}
    Let $f = f_{L/K}$ and $k = \bF_q$, so that $k_L = \bF_{q^f}$. Set $m = q^f - 1$, and let $[\cdot] : \bF_{q^f} \to L$ denote the Teichm\"uller lift for $L$. Let $\zeta_m = [\alpha]$ for some generator $\alpha$ of $\bF_{q^f}^\times$. Then $\zeta_m$ is a primitive $m$th root of unity. Set $K_0 = K(\zeta_m)$, then $K_0 / K$ is Galois, since it is the splitting field of $X^m - 1$. $K_0$ has residue field $k_0 = \bF_q(\alpha) = \bF_{q^f} = k_L$. Let $\res : \Gal(K_0 / K) \to \Gal(k_0 / k)$ be the natural map. For $\sigma \in \Gal(K_0 / K)$, if $\sigma(\zeta_m) \equiv \zeta_m \pmod{\fm_0}$, then $\sigma(\zeta_m) = \zeta_m$, as $\cO_{K_0}^\times \to k_0^\times$ induces a bijection between $m$th roots of unity by Hensel's lemma. Hence $\res$ is injective. Thus $[K_0 : K] = \abs{\Gal(K_0 : K)} \le \Gal(k_0 / k) = f_{K_0 / K}$. We see that the inequality is actually an equality, so $\res$ is an isomorphism, and $K_0 / K$ is unramified.
...
\begin{remark}\phantom{}
    \begin{enumerate}
        ...
        \item $I_{L/K} = \Gal(L/K_0)$, where $K_0$ is as in \autoref{thm:13_3}.
    \end{enumerate}
\end{remark}
...
Let $K / \bQ_p$ be a finite extension, $e = e_{K/\bQ_p}$, and $\pi \in \cO_K$ a uniformiser.
