...
Let $L / K$ be a finite extension. For $x \in L$, we write $\Tr_{L/K}(x) \in K$ for the trace of the $K$-linear map $y \mapsto x y$. If $L/K$ is separable of degree $n$, let $\sigma_1, \ldots, \sigma_n : L \to \overline{K}$ denote the set of distinct embeddings of $L$ into an algebraic closure $\overline{K}$. Then
\begin{equation*}
...
\begin{theorem}\label{thm:10_9}
    The natural map $L \otimes_K K_\fp \to \prod_{\fp \mid \cP} L_\cP$ is an isomorphism.
\end{theorem}
\begin{proof}
...
\begin{example}
    Let $K = \bQ$, $L = \bQ(i)$, $f(X) = X^2 + 1$, $\fp = (5)$, $K_\fp = \bQ_5$. By Hensel's lemma, $\sqrt{-1} \in \bQ_5$. $\bQ(i) \otimes_\bQ \bQ_5 \cong \bQ_5 \otimes_\bQ \bQ[X]/(X^2 + 1) \cong \bQ_5[X]/(X^2+1) \cong \bQ_5[X]/(X-\sqrt{-1}) \times \bQ_5[X]/(X+\sqrt{-1}) \cong \bQ_5 \times \bQ_5$. Thus $5$ splits in $\bQ(i)$ as $5 \cO_L = \fp_1 \fp_2$.
\end{example}
...
Let $\cO_K$ be a Dedekind domain, $K = \Frac{\cO_K}$, $L / K$ a finite separable extension, $\cO_L$ the integral closure of $\cO_K$ in $L$, $\fp$ a non-zero prime ideal of $\cO_K$. Suppose that $\fp$ factors as a product of distinct primes $\fp \cO_L = \cP_1^{e_1} \ldots \cP_r^{e_r}$ in $\cO_L$, where $e_i > 0$.
...
\begin{proof}
    ...
    For the LHS, the structure theorem for finitely-generated modules over PIDs implies that\footnote{since $\cO_L$ is torsion-free} $\cO_L$ is a free $\cO_K$-module of rank $n = [L : K]$. Thus $\cO_L / \fp \cO_L \cong (\cO_K / \fp)^n$ as $\cO_K / \fp$-modules, hence $\dim_K \cO_L / \fp \cO_L = n$.
\end{proof}
