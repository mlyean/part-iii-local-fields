...
\begin{definition}\label{def:18_1}
    ...
    \begin{enumerate}
        \item $F(X, Y) \equiv X + Y \pmod{(X^2, XY, Y^2)}$;
...
Let $K$ be a local field and $q = \abs{k}$.
...
\begin{definition}\label{def:19_2}
    Let $\pi \in \cO_K$ be a uniformiser. A \emph{Lubin-Tate} series for $\pi$ is a power series $f(X) \in \cO_K \llbracket X \rrbracket$ such that
    \begin{enumerate}
