...
\begin{theorem}[Local Kronecker-Weber Theorem]\label{thm:17_4}
...
\begin{lemma}[Key Lemma]\label{lem:19_4}
    Let $f(X), g(X)$ be Lubin-Tate series for $\pi$. Let $L(X_1, \ldots, X_n) = \sum_{i=1}^n a_i X_i$, $a_i \in \cO_K$. Then there is a unique power series $F(X_1, \ldots, X_n) \in \cO_K \llbracket X_1, \ldots, X_n \rrbracket$ such that
    \begin{enumerate}
...
\begin{proof}[Proof of \autoref{thm:19_3}]\phantom{}
    \begin{enumerate}
        \item By \autoref{lem:19_4}, there exists a unique $F_f(X, Y) \in \cO_K \llbracket X, Y \rrbracket$ such that
            ...
            \end{equation*}
            Similarly, $f \circ F_f(F_f(X, Y), Z) = F_f(F_f(f(X), f(Y)), f(Z))$. Thus $F_f(X, F_f(Y, Z)) = F_f(F_f(X, Y), Z)$ by the uniqueness part of \autoref{lem:19_4}.
...
\begin{proof}
    ...
    Assume that $\Char K = p$. We use strong induction on $n \ge 1$.

    Suppose that $h_1(X), \ldots, h_{n-1}(X)$ are irreducible. Then $f_{n-1}(X) = h_1(X) \ldots h_{n-1}(X)$ is a product of separable irreducible polynomials of distinct degrees, hence separable. We have
    \begin{equation*}
